...
### Collecting failure artifacts in GitHub Actions

In CI, make sure the artifact upload step runs after the job that generates the report has completed, so screenshots, logs, and reports can still be collected when `maestro test` fails. Because the JUnit report and the extra debug artifacts have different expectations, it is clearer to upload them in separate steps.
...
- name: Upload Maestro artifacts
  ...
  with:
    ...
    path: |
      build/maestro-artifacts
      build/maestro-debug
    # May be empty on successful runs.
    if-no-files-found: ignore
```

This example keeps the different expectations separate:

| Output | Purpose |
| ------ | ------- |
| `build/maestro-report.xml` | The JUnit report generated by `--format junit --output ...`. |
| `build/maestro-artifacts` | Screenshots, videos, `commands-*.json`, and AI reports from `--test-output-dir`. |
| `build/maestro-debug` | `maestro.log` from `--debug-output`. |

Use `if-no-files-found: error` for the JUnit report because the report is expected whenever `--format junit` is used. For optional artifact directories such as `build/maestro-artifacts` and `build/maestro-debug`, `if-no-files-found: ignore` avoids noisy warnings when a run produces no extra files.

When investigating a failed CI run, start with the JUnit or HTML report to identify the failed Flow, then review `maestro.log` for execution details, screenshots or videos for the device state, and `commands-*.json` for the commands Maestro executed.
