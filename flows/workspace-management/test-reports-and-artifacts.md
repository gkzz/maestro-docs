...
In CI, make sure your artifact upload step still runs when `maestro test` fails. In GitHub Actions, use `if: always()` on the upload step so screenshots, logs, and reports are preserved for failed runs.
...
This example keeps the three output types separate:
...
Use `if-no-files-found: warn` when uploading optional artifact directories. Depending on the run and the commands used in your Flows, a directory such as `build/maestro-artifacts` can be empty. Without this setting, the upload step can fail even when `maestro test` itself succeeded.
